...
public class DataPrePro {
	...
	private static String data18 = "F:/系数计算/cq系数计算";
	private static String data18res = "F:/系数计算/res";
	...
	public static void preData(String inPath, String outPath) {
		...
		for (int i = 0; i < list.size(); i++) {
			// 依次处理每一个文件
			String pathIn = inPath + "/" + list.get(i);
			...
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					if(lineArray.length >= 51) {
						...
						double travelTime = timeInterval/(double)1000/60/60;
		...
		System.out.println("******************18-19年数据预处理完毕*************");
	}
